...
const UploadDocuments = () => {
  ...
  const [submissionStatus, setSubmissionStatus] = useState(null); // To hold the verification message
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/salon/upload-documents/${salonId}`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        navigate(`/document-verification/${result.referenceId}`); // Redirect to a confirmation page
      }
    } catch (error) {
      alert('Error uploading documents');
